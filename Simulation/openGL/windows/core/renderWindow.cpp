...
namespace OpenGL
{
	FreeCamWindow::FreeCamWindow()
		: _shader("openGL/shaders/vert.vert", "openGL/shaders/frag.frag"),
		_lastCash(App::_lastCash),
	...
	bool FreeCamWindow::render()
	{
		...
		for (auto [id, obj] : _lastCash.objects) {
			glUniformMatrix4fv(2, 1, GL_FALSE, &obj.mat4()[0][0]);
			objectLists::modelCash.getOpenGLModel(id)->render();
		}
