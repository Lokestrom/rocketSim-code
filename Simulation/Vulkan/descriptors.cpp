...
DescriptorSetLayout::~DescriptorSetLayout() {
    _device.device().destroyDescriptorSetLayout(_descriptorSetLayout);
}
...
void DescriptorPool::freeDescriptors(std::vector<vk::DescriptorSet>& descriptors) const {
    _device.device().freeDescriptorSets(_descriptorPool,
        static_cast<uint32_t>(descriptors.size()),
        descriptors.data());
}

void DescriptorPool::resetPool() {
    _device.device().resetDescriptorPool(_descriptorPool);
}
