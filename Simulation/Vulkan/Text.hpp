...
public:
	...
	public:
		...
		VaryingTextString(VaryingTextString&& other) noexcept
		...
		template<typename T>
		void addFunction(T(*func)(), sizeT index);
		void assignText(std::string text);
...
public:
	...
	VaryingText(VaryingText&& other) noexcept 
	...
	template<typename T>
	void addVariable(T& var, sizeT index);
	//may add the ability to call function with parameters later
	template<typename T>
	void addFunction(T(*func)(), sizeT index);
	void assignText(std::string text);
...
{
	Error("VaryingText::VaryingTextString::addVariable: No specialization for this type (fault can resolt form a call of the VaryingText::addVariable function)", Error::Type::codeFault, Error::recoveryType::unrecoverable);
}
template<>
...
template<typename T>
inline void VaryingText::VaryingTextString::addFunction(T(*func)(), sizeT index)
{
	Error("VaryingText::VaryingTextString::addFunction: No specialization for this type (fault can resolt form a call of the VaryingText::addFunction function)", Error::Type::codeFault, Error::recoveryType::unrecoverable);
}

template<>
inline void VaryingText::VaryingTextString::addFunction(Vector3(*func)(), sizeT index)
{
	_variable.pushBack({ func, Variable::Type::FungtionVec3, index });
	_variable.bubbleSort();
}
...
template<typename T>
inline void VaryingText::addFunction(T(*func)(), sizeT index)
{
	_text.addFunction(func, index);
}
