...
bool TimeWindow::render()
{
	...
	if (App::simulationRunning()) {
		ImGui::Text("Time:: %f s", App::getSimulationRunTime());
		ImGui::Text("Simulated time: %f s", App::getSimulationContainer().getSimulation().getTime());
		if (App::getSimulationContainer().getSimulation().getTime() > App::getSimulationRunTime())
			ImGui::Text("Ahead");
		else
			ImGui::Text("Behind");
		ImGui::Text("Cashed frames: %u", App::getSimulationContainer().getRenderingCache().size());
